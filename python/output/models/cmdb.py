...
database = MySQLDatabase('cmdb', **db_config) 
...
class Device(BaseModel):
    ...
    operator = CharField()
    rack = ForeignKeyField(db_column='rack_id', rel_model=Rack, to_field='rack_id')
    remarks = CharField(null=True)
    room = ForeignKeyField(db_column='room_id', null=True, rel_model=Room, to_field='room')
    seat = IntegerField()
...
class DeviceTemplate(BaseModel):
    ...
    manufacturer = IntegerField(db_column='manufacturer_id', index=True, null=True)
...
class Seat(BaseModel):
    create_timestamp = DateTimeField(null=True)
    rack = ForeignKeyField(db_column='rack_id', null=True, rel_model=Rack, to_field='rack_id')
    room = ForeignKeyField(db_column='room_id', null=True, rel_model=Room, to_field='room')
...
class Segment(BaseModel):
    ...
    class Meta:
        db_table = 'segment'
        indexes = (
            (('segment_ip', 'netmask'), True),
        )

class SegmentIpPool(BaseModel):
    assigned = CharField()
    ip = CharField(unique=True)
    segment = ForeignKeyField(db_column='segment_id', rel_model=Segment, to_field='segment')

    class Meta:
        db_table = 'segment_ip_pool'

class ServerTag(BaseModel):
    assets = ForeignKeyField(db_column='assets_id', null=True, rel_model=Device, to_field='assets')
    create_timestamp = DateTimeField()
    ...
    class Meta:
        db_table = 'server_tag'
        indexes = (
            (('server_tag_key', 'server_tag_value', 'assets'), True),
        )

class ServerTagUser(BaseModel):
    create_timestamp = DateTimeField()
    server_tag = ForeignKeyField(db_column='server_tag_id', null=True, rel_model=ServerTag, to_field='server_tag')
